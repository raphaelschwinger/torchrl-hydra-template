# Adapted from https://github.com/nicklashansen/tdmpc2 (tdmpc2/common/scale.py),
# MIT license. Changes: hardcoded cuda:0 replaced by a `device` argument;
# `cfg` replaced by an explicit `tau` argument.
"""Running trimmed scale estimator (5th-95th percentile range, EMA-smoothed)."""
...
class RunningScale(torch.nn.Module):
    """Tracks the trimmed value range of a stream of tensors for normalization."""

    def __init__(self, tau: float, device: torch.device | str = "cpu") -> None:
        super().__init__()
        self.tau = tau
